...
inline void Ranges_MakeRangeConst()
{
	...
	// adding std::as_const() makes the container as constant if const std::array therefore the function that
	// view is passed will not be able to change the elements
	std::ranges::take_view tkView{ std::as_const(arr1), 4 };
